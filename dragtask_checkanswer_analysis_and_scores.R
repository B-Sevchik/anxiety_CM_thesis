...
# STEP 1: calculating the data frame for the analysis (note we're not saving this right now)
... <- average_scores %>% 
  mutate(threattype = ifelse(grepl("threat", src), "threat", "neutral")) %>% 
  group_by(subject, threattype) %>% 
  summarise(mean_score3 = mean(score3))

#STEP 2: join in each subjects anxiety score
# example of joining in anxiety group
example_data_df <- tibble(
  subject = c(1:10),
  score = rnorm(10, 10, 2)
)

#this you need to acquire, a df that has for each subject, their anxiety group
example_subject_anxiety <- tibble(
  subject = c(1:10),
  anxiety = sample(c(1, 2), 10, replace=TRUE)
)

# then join that data frame with 
example_data_df %>% 
  left_join(example_subject_anxiety, by="subject")
